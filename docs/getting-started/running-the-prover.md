...
Proving a single block on GPU is done with service mode, bounded to one block:
...
docker run --rm --gpus all --network host \
  ...
  --service --rpc-url "$RPC_URL" --data-dir /work/data \
  --start-block 23100000 --end-block 23100000 \
  --prove-every 1 --proof-type compressed
```

The proof is written to `<data-dir>/<block>/proof<block>.bin` — here
`/work/data/23100000/proof23100000.bin`. Service mode fetches the block and its witness itself, so no separate
`fetch` run is needed.

:::warning
Use service mode, not the one-shot `prove` subcommand, whenever you need the proof **on disk**. `prove` keeps the
proof in memory and never writes it, ignores `--proof-path`, and exits 0 even when proving fails — see
[`prove` in the CLI reference](cli-reference.md#prove). Two flags above are load-bearing: `SP1_PROVER=cuda` (any
other value, or unset, silently selects the CPU prover) and `--prove-every 1` (it is a modulo on the block number
and defaults to *never*, so omitting it proves nothing).
:::
...
To keep proving continuously instead, drop the block bounds and widen the interval — with `--prove-every 100`, only
blocks whose number is divisible by 100 are proved:
